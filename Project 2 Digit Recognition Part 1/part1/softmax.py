...
def compute_probabilities(X, theta, temp_parameter):
    ...
    c = np.amax((X.dot(theta.transpose()))/temp_parameter, axis=1).reshape(-1,1)
    z = np.exp((X.dot(theta.transpose())/temp_parameter)-c) 
    my_sum = np.sum(z,axis=1).reshape(-1,1)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    y_predicted_mod3 = np.mod(y_predicted, 3)
    return 1- np.mean(y_predicted_mod3 == Y)
