...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    n_mu_parameters = mu_s.shape[0]* mu_s.shape[1]
    n_var_parameters = var_s.shape[0]
    n_p_parameters = p_s.shape[0] -1 

    p = n_mu_parameters + n_var_parameters + n_p_parameters
    n,_ = X.shape
    l = log_likelihood
    BIC = l - 0.5*p*np.log(n)
